...
/**
 * Modified from https://github.com/esyede/single-blade
...
class View
{
    ...
    public function __construct()
    {
        ...
        $this->setViewFolder(dirname(__DIR__) . DS . 'views');
        $this->setCacheFolder(dirname(__DIR__) . DS . 'cache');
    ...
    public function clearCache()
    {
        ...
        $files = glob($this->cacheFolder . DS . '*.' . $extension);
        $result = true;
    ...
    {
        $this->viewFolder = str_replace('/', DS, $path);
    }
    ...
    {
        $this->cacheFolder = str_replace('/', DS, $path);
    }
    ...
    {
        $view = str_replace(['.', '/'], DS, ltrim($view, '/'));
        $actual = $this->viewFolder . DS . $view . $this->fileExtension;

        $view = str_replace(['/', '\\', DS], '.', $view);
        $cache = $this->cacheFolder . DS . $view . '__' . sprintf('%u', crc32($view)) . '.php';
